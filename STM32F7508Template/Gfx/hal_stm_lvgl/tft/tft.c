...
#include "../../../Gfx/lv_conf.h"
#include "../../../Gfx/lvgl/lvgl.h"
...
/* You can try to change buffer to internal ram by uncommenting line below and commenting SDRAM one. */

//see STM32F750N8Hx_FLASH.ld ,this frame buffer end up in sdram 0xc0000000
static uintpixel_t __attribute__((section (".ram"))) my_fb[TFT_HOR_RES * TFT_VER_RES] ;
//

//static __IO uintpixel_t * my_fb = (__IO uintpixel_t*) (0xc0010000);
...
void tft_init(void)
{
   ...
   /* LittlevGL requires a buffer where it draws the objects. The buffer's has to be greater than 1 display row*/
	...
	static lv_color_t buf1_1[TFT_HOR_RES * 17];// 68];
	static lv_color_t buf1_2[TFT_HOR_RES * 17];// 68];
	lv_disp_draw_buf_init(&disp_buf_1, buf1_1, buf1_2, TFT_HOR_RES *17);//68);   /*Initialize the display buffer*/
...
#endif
    err = HAL_DMA_Start_IT(&DmaHandle,(uint32_t)buf_to_flush, (uint32_t)&my_fb[y_fill_act * TFT_HOR_RES + x1_flush], length);
    if(err != HAL_OK)
    {
...
static uint8_t LCD_Init(void)
{
    ...
    // We are already using SDRAM, it's initialise in ExtMem_Boot Projet, no need to initialise it again
    //BSP_SDRAM_Init();
